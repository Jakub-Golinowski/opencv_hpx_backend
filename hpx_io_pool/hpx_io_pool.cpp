...
void show_image(const cv::Mat& image, std::string win_name)
{
    // show_image is executed on opencv tp, therefore OS thread ID shoudl be
    // different than the IDs of OS-threads for default pool
    hpx::cout << "show_image in window " << win_name
              << " hpx_thread = " << std::hex << hpx::this_thread::get_id()
              << " os_thread = " << std::hex << std::this_thread::get_id()
              << std::endl;
...
{
    // save_image is executed on opencv tp, therefore OS thread ID shoudl be
    // different than the IDs of OS-threads for default pool
...
int hpx_main(boost::program_options::variables_map& vm)
{
    ...
    std::size_t num_pixels =
        (std::size_t) mandelbrotImg.rows * mandelbrotImg.cols;
    std::size_t chunk_size =
        (std::size_t) round(((double) num_pixels / num_work_threads));
    hpx::parallel::execution::static_chunk_size fixed(chunk_size);
    hpx::lcos::local::mutex m;
    int hpx_thread_counter = 0;
    ...
    hpx::parallel::for_loop_strided(
        ...
        num_pixels, 1, [&](std::size_t r) {
            ...
            if (r % chunk_size == 0)
            {
                // Lock added to keep print messages in order
                std::lock_guard<hpx::lcos::local::mutex> lock(m);
                // Print message should be printed out once from each chunk
                // We can see that all the OS threads assigned to default pool
                // are used.
                hpx::cout << hpx_thread_counter++
                          << ". hpx_thread = " << std::hex
                          << hpx::this_thread::get_id()
                          << " os_thread = " << std::hex
                          << std::this_thread::get_id()
                          << " Mandelbrot_loop r = " << std::dec << r
                          << std::endl;
            }
        });

    hpx::future<void> f_save_image = hpx::async(
        io_pool_executor, &save_image, mandelbrotImg, "mandelbrot.bmp");
    hpx::async(io_pool_executor, &show_image, mandelbrotImg, "Mandelbrot");
...
int main(int argc, char* argv[])
{
    ...
    desc_cmdline.add_options()("mb-size", po::value<int>()->default_value(500),
